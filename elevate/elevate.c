...
void main()
{
	...
	if (fShowUsage || ((fNoPushD || fUnicode) && mode == MODE_NORMAL) || (*pszCmdLine == 0 && mode != MODE_CMD_K))
	{
		...
		_tprintf(szUsageTemplateGeneral, TEXT('c'), "Launches a terminating command processor; equivalent to \"cmd /c command\"");
		_tprintf(szUsageTemplateGeneral, TEXT('k'), "Launches a persistent command processor; equivalent to \"cmd /k command\"");
		_tprintf(szUsageTemplateGeneral, TEXT('n'), "When using -c or -k, do not pushd the current directory before execution");
		_tprintf(szUsageTemplateGeneral, TEXT('u'), "When using -c or -k, use Unicode; equivalent to \"cmd /u\"");
		_tprintf(szUsageTemplateGeneral, TEXT('w'), "Waits for termination; equivalent to \"start /wait command\"");
